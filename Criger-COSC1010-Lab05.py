...
# Lab 03 
# Lab Section: 10
# Sources, people worked with, help given to: Kaylee Esgar and Austin Barner (TA) and other TA I am not sure what his name is
# your
# comments
# here
...
max_temp = max_temps[0] # Starts reading the first number in the list
min_temp = min_temps[0] # Starts reading the first number in the list

for temp in max_temps: # starts a loop 
    if temp > max_temp: # checks if the temp is greater then max_temp
        max_temp = temp # If temp > max_temp then max_temp will now equal temp
print(f"Max temp = {max_temp}") # Prints value of max_temp

for temp in min_temps: # starts a loop
    if temp < min_temp: # checks if the temp is less then min_temp
        min_temp = temp # If temp < min_temp then min_temp will now equal temp
print(f"Min temp = {min_temp}") # Prints value of max_temp
...
pos_count = 0 # Starts a counter starting at 0
neg_count = 0 # Starts a counter starting at 0
zero_count = 0 # Starts a counter starting at 0
for number in numbers: # creates a loop 
    if number > 0: # Checks if the number is positive
        pos_count = pos_count + 1
    elif number < 0: # Checks if the number is negative
        neg_count = neg_count + 1
    else: # States if not positive or negative (=0) then it will count as zero count
        zero_count = zero_count + 1
print(f'There are {pos_count} positive numbers') # Prints the count of positive numbers
print(f'There are {neg_count} negative numbers') # Prints the count of negative numbers
print(f"Zero occurred {zero_count} time(s)") # Prints the count that zero occurs
...
pos_sum = 0 # Starts a count for positive summing
neg_sum = 0 # Starts a count for negative summing
for number in numbers: # starts a loop to check conditions
    if number > 0: # Checks if the number is positive
        pos_sum = pos_sum + number # If positive it will go through the list and the number listed in numbers
    elif number < 0: # If not positive it goes to the next condition to check if it is negative
        neg_sum = neg_sum + number # If negative it will go through the list and the number listed in numbers

print(f"Sum of positive numbers {pos_sum}") # Prints the sum of positive numbers
print(f"Sum of negative numbers {neg_sum}") # Prints the sum of negative numbers
